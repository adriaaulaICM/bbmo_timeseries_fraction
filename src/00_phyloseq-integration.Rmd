...
tax.raw <- readRDS("../data/abund-tax-raw/devotes_idtaxa_tax_assignation.rds") %>% 
  data.frame() %>% 
...
colnames(tax.raw) <- c('Domain', 'Phylum', 'Class', 'Order', 'Family', 'Genus', 'Species', 'asv', 'seq')
```
...
tax.raw <- tax.raw %>% 
  ...
  mutate(curated = ifelse(Phylum %in% c("Actinobacteriota", 
         ...
         curated = ifelse(curated == "Alphaproteobacteria" &
                            Order %in% c("Rhodobacterales",
                                         ...
                                         "SAR11 clade","Sphingomonadales"),
                          Order,curated),
         curated = ifelse(curated == "Alphaproteobacteria", "Other Alphaproteobacteria",curated),
         curated = ifelse(curated=="Gammaproteobacteria" &
                            Order %in% c( "Pseudomonadales",
                                          "Cellvibrionales",
                                          "Oceanospirillales",
                                          'Enterobacterales'),
                          Order, curated),
         curated = ifelse(Order=="Alteromonadales" &
                            Family %in% c("Alteromonadaceae"),
                          Family, curated),
         curated = ifelse(curated %in% c("Gammaproteobacteria", 'Enterobacterales', 'Pseudomonadales'),
                          "Other Gammaproteobacteria",curated),
         curated = ifelse(curated=="Bacteroidota" &
                            Order == "Flavobacteriales", Order,curated),
         curated = ifelse(curated=="Bacteroidota", "Other Bacteroidota",curated),
         curated = ifelse(curated=="Cyanobacteria" &
                            Family %in% "Cyanobiaceae",
                          Family, curated),
         curated = ifelse(curated=="Cyanobacteria","Other Bacteria",curated)) 
...
tax.raw$curated <- gsub("Alteromonadaceae",  "Gam-Alteromonadaceae", tax.raw$curated)
...
tax.raw$curated <- gsub("SAR11 clade",  "Alp-SAR11 clade", tax.raw$curated)
...
nich.df <- read_rds('../data/robjects/nicheval.rds')
